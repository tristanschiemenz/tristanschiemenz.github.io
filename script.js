...
var cardData = JSON.parse(ScardData);
...
function displayCards(randomNumbers) {
    const container = document.getElementById('cards-container');
    while (container.firstChild) {
        container.removeChild(container.firstChild);
    }
    // Loop through the cards and create image elements
    for (let i = 0; i < 8 && i < cards.length; i++) {
        const card = cards[randomNumbers[i]];
        ...
        img.src = imgUrl;
        img.alt = card.name;
        img.style.width = '100px'; // Set the width or any style you need
...
function createCustomLink(randomNumbers){
    ...
    customLink = basicString;
}
function openCustomeLink() {
...
var cards = cardData.items; // lenght 0 - 109
var numberOfItems = cards.length;
var customLink = "";
